...
class bb_node:
    def __init__(self,value,room,index,items,parent_edge,max_estimate):
        self.value = value
        self.room = room
        self.items = items
        self.index = index
        ...
        self.estimate = self.get_estimate(max_estimate)


    def get_estimate(self, max_estimate):
        estimate = max_estimate
        current_node = self
        while current_node.parent_edge != None:
            if current_node.parent_edge.take == 0:
                multiplier = 1
            else:
                multiplier = 0
            estimate -= multiplier * self.items[current_node.parent_edge.item_index]
            current_node = parent_edge.end
        return estimate
...
def solve_bb(item_count, capacity, items, taken):
    ...
    #create our root node with the first item in the list
    root = bb_node(0,capacity,0,items,None,max_estimate)
    root.show()

    root.left_edge = bb_edge(root.index + 1, 1, root, bb_node(items[root.index + 1].value + root.value, root.room - items[root.index + 1].weight,root.index + 1,root.items,
